...
class TaskUtilsTest(unittest.TestCase):
  ...
  def testGeneral(self):
    ...
    id_1 = task_manager.Start(input_1, 0.2)
    id_2 = task_manager.Start(input_2, 0.2)

    time.sleep(0.5)
    read_list_1_1 = list(task_manager.Read(id_1))
    read_list_2_1 = list(task_manager.Read(id_2))
    ...
    time.sleep(0.5)
    read_list_1_2 = list(task_manager.Read(id_1))
    read_list_2_2 = list(task_manager.Read(id_2))
    ...
    time.sleep(0.5)
    read_list_1_3 = list(task_manager.Read(id_1))
    self.assertRaisesRegexp(
        AssertionError, 'No task with id %s' % id_2,
        list, task_manager.Read(id_2))
    ...
    time.sleep(0.5)
    self.assertRaisesRegexp(
        AssertionError, 'No task with id %s' % id_1,
        list, task_manager.Read(id_1))
    self.assertRaisesRegexp(
        AssertionError, 'No task with id %s' % id_2,
        list, task_manager.Read(id_2))
